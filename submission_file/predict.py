...
trained_model, preprocessing = model.load_model()

if os.getenv("WEEKLY_EVALUATION", "false") == "true":
	prices = model.predict_premium(trained_model, Xraw, preprocessing)
	np.savetxt(output_prices_file, prices, delimiter=',', fmt='%.5f')
else:
	claims = model.predict_expected_claim(trained_model, Xraw, preprocessing)
